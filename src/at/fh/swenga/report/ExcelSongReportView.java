...
public class ExcelSongReportView extends AbstractXlsxView {
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		// create a worksheet
		Sheet sheet = workbook.createSheet("Music Report");
		...
		cell3.setCellValue("Album");
		cell3.setCellStyle(style);
		
		// create a new cell in the row 
		Cell cell4 = headerRow.createCell(4);
		cell4.setCellValue("Release date");
		cell4.setCellStyle(style);
		
 
		// create multiple rows with song data
		int rowNum = 1;
		for (SongModel song : songs) {
			// create the row data
			Row row = sheet.createRow(rowNum++);
			row.createCell(0).setCellValue(song.getId());
			row.createCell(1).setCellValue(song.getSongName());
			row.createCell(2).setCellValue(song.getArtist());
			row.createCell(3).setCellValue(song.getAlbum());
			row.createCell(4).setCellValue(song.getReleaseDate().toString());
		}
